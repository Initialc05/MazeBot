...
// ==================== 编码器里程计配置 ====================
#define USE_ENCODER_ODOM true  // true=编码器里程计, false=IMU里程计（向后兼容）

// 小车物理参数（需要实测后填入准确值）
#define WHEEL_DIAMETER 0.065   // 轮径（米）- 默认65mm，请实测后修改
#define WHEEL_BASE 0.100       // 轮距（米）- 默认100mm，请实测后修改
#define TICKS_PER_REV 390      // 编码器每圈脉冲数（13线×30减速比=390）
#define DEG_TO_RAD 0.017453292519943295  // 度转弧度常量（PI/180）

// 编码器里程计缩放系数（用于微调精度）
#define ENCODER_SCALE_FACTOR 0.534  // 缩放系数，1.0=无缩放，>1.0=放大，<1.0=缩小

// 编码器里程计全局变量
extern float encoder_odom_x;      // 编码器计算的X位移（米，IMU坐标系）
extern float encoder_odom_y;      // 编码器计算的Y位移（米，IMU坐标系）
extern void updateEncoderOdometry();  // 编码器里程计更新函数
...
// ==================== IMU时间戳缓冲区（用于与雷达数据对齐）==================== 
#define IMU_BUFFER_SIZE 100  // 缓冲区大小：100个数据点（500Hz下约0.2秒历史）
 ...
 */
struct IMUSnapshot {
    uint32_t timestamp_ms;  // 系统时间戳（millis()）
    float x;                // 位移X（米）
    float y;                // 位移Y（米）
    float theta;            // 朝向角度（度）
};
...
extern volatile uint8_t imu_buffer_head;   // 写指针（最新数据位置）
extern volatile uint8_t imu_buffer_tail;   // 读指针（最旧数据位置）

/**
 * 根据时间戳查找最接近的IMU数据
 * @param target_timestamp_ms 目标时间戳
 * @param matched_snapshot 输出：匹配到的IMU数据（引用传递）
 * @return true=找到匹配数据, false=缓冲区为空
 */
extern bool getIMUSnapshotByTimestamp(uint32_t target_timestamp_ms, IMUSnapshot& matched_snapshot);
...
extern void Cmd_41(void);     // 读取 开机后工作模式
...
extern void Cmd_44(void);     // 读取 自动补偿高度标识
...
extern void Cmd_48(void);     // 读取 串口波特率
extern void Cmd_49(void);     // 闪烁几下led指示灯
