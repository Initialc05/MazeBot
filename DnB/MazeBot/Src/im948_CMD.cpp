...
extern HardwareSerial IMU900Serial;
...
// ==================== 编码器里程计全局变量 ====================
float encoder_odom_x = 0.0;    // 编码器计算的X位移（米，IMU坐标系）
float encoder_odom_y = 0.0;    // 编码器计算的Y位移（米，IMU坐标系）
long last_left_ticks = 0;      // 上次左轮编码器脉冲数
long last_right_ticks = 0;     // 上次右轮编码器脉冲数
...
/**
 * 根据时间戳查找最接近的IMU数据
 * 算法：在环形缓冲区中二分查找最接近目标时间戳的数据
 * 
 * @param target_timestamp_ms 目标时间戳（雷达点采样时刻）
 * @param matched_snapshot 输出：匹配到的IMU数据
 * @return true=找到匹配, false=缓冲区为空
 */
bool getIMUSnapshotByTimestamp(uint32_t target_timestamp_ms, IMUSnapshot& matched_snapshot) {
  // 检查缓冲区是否为空
  if (imu_buffer_head == imu_buffer_tail) {
    // 缓冲区为空，返回当前IMU数据作为备选
    matched_snapshot.timestamp_ms = millis();
    matched_snapshot.x = OffsetX;
    matched_snapshot.y = OffsetY;
    matched_snapshot.theta = AngleZ;
    return false;
  }
  
  // 简化策略：遍历缓冲区找到时间戳最接近的数据
  uint8_t best_idx = imu_buffer_head;
  uint32_t min_time_diff = 0xFFFFFFFF;  // 最大值
  
  uint8_t idx = imu_buffer_tail;
  while (idx != imu_buffer_head) {
    // 计算时间差（绝对值）
    uint32_t time_diff = (imu_buffer[idx].timestamp_ms > target_timestamp_ms) ?
                         (imu_buffer[idx].timestamp_ms - target_timestamp_ms) :
                         (target_timestamp_ms - imu_buffer[idx].timestamp_ms);
  ...
  // 检查head位置的数据
  uint32_t time_diff = (imu_buffer[imu_buffer_head].timestamp_ms > target_timestamp_ms) ?
                       (imu_buffer[imu_buffer_head].timestamp_ms - target_timestamp_ms) :
                       (target_timestamp_ms - imu_buffer[imu_buffer_head].timestamp_ms);
  if (time_diff < min_time_diff) {
  ...
  // 输出匹配结果
  matched_snapshot = imu_buffer[best_idx];
  
  // 如果时间差超过200ms，可能数据太旧，打印警告
  #if DEBUG_PRINT_ENABLED
  if (min_time_diff > 200) {
    Serial.print("[IMU] Warning: Time mismatch=");
    Serial.print(min_time_diff);
    Serial.println("ms");
  }
  #endif
...
// IMU900 初始化
void initIMU900() {
  // 注意：不要重复初始化Serial，setup()中已经初始化过了
  Dbp("正在初始化IMU900...\r\n");
...
// ==================== 独立Odom数据包发送 ====================
// 独立Odom数据包结构（11字节，区别于雷达融合包18字节）
struct __attribute__((packed)) OdomOnlyPacket {
  uint16_t header;                 // 固定头：0xBB55（区别于雷达包0xAA55）
...
/**
 * 发送独立Odom数据包（高优先级，零延迟传输）
 * 
 * 优势：
 * 1. 数据包小（11字节 vs 18字节），发送更快
 * 2. 不受雷达数据传输速度限制
 * 3. IMU 250Hz更新时立即发送，零延迟
 * 4. 上位机可以实时获取最新位姿
 */
void sendOdomUpdate() {
  #if USE_BLUETOOTH_MODE
  OdomOnlyPacket odom_pkt;
  odom_pkt.header = 0xBB55;  // 独立包头（区别于0xAA55）
  
  // ========== 数据源选择 ==========
  #if USE_ENCODER_ODOM
    // 新版：编码器位移 + IMU朝向（推荐）
    odom_pkt.odom_x_cm = (int16_t)(encoder_odom_x * 100);  // 编码器X → 厘米
    odom_pkt.odom_y_cm = (int16_t)(encoder_odom_y * 100);  // 编码器Y → 厘米
  #else
    // 旧版：纯IMU（向后兼容）
    odom_pkt.odom_x_cm = (int16_t)(OffsetX * 100);         // IMU X → 厘米
    odom_pkt.odom_y_cm = (int16_t)(OffsetY * 100);         // IMU Y → 厘米
  #endif
  
  // 朝向始终使用IMU（准确可靠）
  odom_pkt.odom_theta_deg_q8 = (int32_t)(AngleZ * 256);   // 度 → Q8定点数
  
  // 计算校验和（XOR所有字节，除了最后的checksum）
  uint8_t* packet_bytes = (uint8_t*)&odom_pkt;
  odom_pkt.checksum = 0;
  for (uint8_t i = 2; i < sizeof(OdomOnlyPacket) - 1; i++) {
    odom_pkt.checksum ^= packet_bytes[i];
  }
  
  // 发送数据包（11字节二进制）
  DATA_SERIAL.write((uint8_t*)&odom_pkt, sizeof(OdomOnlyPacket));
  #endif
}
 ...
 * 2. 计算轮子移动距离（使用轮径参数）
 * 3. 差速法计算小车中心移动距离
 * 4. 使用IMU的AngleZ作为准确朝向
 * 5. 更新世界坐标系累积位置（IMU坐标系：X前，Y左）
 * 
 * 优势：
 * - 旋转时无位移漂移（不受离心力影响）
 * - 短距离精度高（±1cm/米）
 * - 保持IMU朝向优势
 */
void updateEncoderOdometry() {
  // 1. 读取当前编码器值
  long left_ticks = getLeftTicks();
  long right_ticks = getRightTicks();
  
  // 2. 计算增量（相对上次更新）
  long delta_left = left_ticks - last_left_ticks;
  long delta_right = right_ticks - last_right_ticks;
  
  // 🛡️ 防护：检测异常大的跳变（可能是硬件故障）
  const long MAX_REASONABLE_TICKS = 1000;  // 2ms内最大合理脉冲数（约2.5圈）
  if (abs(delta_left) > MAX_REASONABLE_TICKS || abs(delta_right) > MAX_REASONABLE_TICKS) {
    // 检测到异常跳变，跳过本次更新
    return;
  }
  
  // 3. 转换为轮子实际移动距离（米）
  //    公式：距离 = (脉冲数 / 每圈脉冲数) × 轮周长 × 缩放系数
  //    轮周长 = π × 轮径
  float left_dist = (delta_left / (float)TICKS_PER_REV) * PI * WHEEL_DIAMETER * ENCODER_SCALE_FACTOR;
  float right_dist = (delta_right / (float)TICKS_PER_REV) * PI * WHEEL_DIAMETER * ENCODER_SCALE_FACTOR;
  
  // 4. 差速轮运动学：计算小车中心移动距离和转向
  //    中心移动距离 = (左轮 + 右轮) / 2
  float delta_dist = (left_dist + right_dist) / 2.0;
  
  // 5. 获取当前朝向（使用IMU的AngleZ，准确可靠）
  //    AngleZ是度数，需要转换为弧度用于三角函数
  float theta_rad = AngleZ * DEG_TO_RAD;  // 度转弧度
  
  // 6. 更新世界坐标系位置（IMU坐标系：X前进，Y左移）
  //    新位置 = 旧位置 + 移动距离 × cos/sin(朝向)
  encoder_odom_x += delta_dist * cos(theta_rad);  // 前进方向
  encoder_odom_y += delta_dist * sin(theta_rad);  // 左移方向
...
void updateIMU900() {
  ...
  if (isNewData) {
    ...
    uint8_t next_head = (imu_buffer_head + 1) % IMU_BUFFER_SIZE;
    
    // 如果缓冲区满了，移动tail指针（覆盖最旧数据）
    if (next_head == imu_buffer_tail) {
      imu_buffer_tail = (imu_buffer_tail + 1) % IMU_BUFFER_SIZE;
    }
    
    // 写入新数据
    imu_buffer[next_head].timestamp_ms = current_time;
    imu_buffer[next_head].x = OffsetX;
    imu_buffer[next_head].y = OffsetY;
    imu_buffer[next_head].theta = AngleZ;
    
    // 更新头指针
    imu_buffer_head = next_head;

    // 🚀 更新编码器里程计（在发送前，确保使用最新AngleZ）
    #if USE_ENCODER_ODOM
      updateEncoderOdometry();
    #endif

    // 🚀 立即发送独立Odom数据包（高优先级，零延迟传输）
    // IMU每次更新（250Hz）都会立即发送，上位机可以实时获取最新位姿
    sendOdomUpdate();

    // 计算归一化角度（0-360度）
    float theta_normalized = AngleZ;
    while (theta_normalized < 0) {
      theta_normalized += 360.0f;
    }
    while (theta_normalized >= 360.0f) {
      theta_normalized -= 360.0f;
    }

    // 打印IMU累计位移和角度（原始值 + 归一化值）
    // Serial.print("Odom - X: ");
    // Serial.print(OffsetX, 3);
    // Serial.print("m, Y: ");
    // Serial.print(OffsetY, 3);
    // Serial.print("m, Theta(Raw): ");
    // Serial.print(AngleZ, 2);
    // Serial.print("° → Theta(0-360): ");
    // Serial.print(theta_normalized, 2);
    // Serial.println("°");
    ...
    // BTSerial.print("m, Theta(Raw): ");
    // BTSerial.print(AngleZ, 2);
    // BTSerial.print("° → Theta(0-360): ");
    // BTSerial.print(theta_normalized, 2);
    // BTSerial.println("°");
  }
}
...
int Cmd_PackAndTx(U8 *pDat, U8 DLen) {
  ...
  if ((DLen == 0) || (DLen > CmdPacketMaxDatSizeTx) || (pDat == NULL)) {  // 非法参数
    return -1;
  }
  ...
  switch (RxIndex) {
    ...
    case 5:  // 结束码
      ...
      if (byte == CmdPacket_End) {  // 捕获到完整包
        ...
        if ((targetDeviceAddress == cmdAddress) || (targetDeviceAddress == 255)) {  // 地址匹配，是目标设备发来的数据 才处理
          Dbp_U8_buf("rx: ", "\r\n",
                     "%02X ",
                     buf, i);
...
static void Cmd_RxUnpack(U8 *buf, U8 DLen) {
  ...
  switch (buf[0]) {
    ...
    case 0x17:              // 加速计高精度校准 回复
      if (buf[1] == 255) {  // 字节1 值255 表示采集完成，正在结束校准(设备需继续保持静止等待10秒钟)
