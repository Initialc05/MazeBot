...
NEAR_CORRECTION_FACTOR = 1.0         # 近距离校正系数：1.0表示不缩放近距离
                                    # < 1.0: 近距离点向内收缩（修正外凸）
                                    # > 1.0: 近距离点向外扩展
# 平滑过渡参数
...
class LidarVisualizer:
    def __init__(self, serial_receiver, use_multithreading=False):
        ...
        self.map_xlim = (map_center - map_view_range, map_center + map_view_range)
        self.map_ylim = (map_center - map_view_range, map_center + map_view_range)
        self.map_axes_position = None
        ...
        self.map_axes_position = self.ax2.get_position().frozen()
        self.lock_map_view()

    def lock_map_view(self):
        """Keep the SLAM map from autoscaling down to a thin strip."""
        self.ax2.set_autoscale_on(False)
        self.ax2.set_xlim(*self.map_xlim)
        self.ax2.set_ylim(*self.map_ylim)
        self.ax2.set_aspect('equal', adjustable='box', anchor='C')
        if self.map_axes_position is not None:
            self.ax2.set_position(self.map_axes_position)
